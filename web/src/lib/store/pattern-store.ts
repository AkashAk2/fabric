...
async save(name: string, patternObj: Pattern) {
  try {
    // Prefer same-origin relative API calls from the browser so nginx (or the
    // host) can proxy requests and avoid CORS. If this code runs server-side or
    ...
    if (typeof window !== 'undefined') {
      ...
      url = `${apiRoot}/patterns/${encodeURIComponent(name)}`;
    }
    ...
    // rethrow so calling code (UI) can handle/display the error
    throw error;
  }
}
